...
class HandlerManager(object):
    ...
    def __init__(self):
        ...
        temp = self.load_classes(BaseCommand, 'commands')
        for cmd_cls in temp:
            cmd = cmd_cls(self.handlers)
            self.commands[cmd.arg] = cmd

    def load_classes(self, base_class, package_name):
        classes = []
            ...
            if base != '__init__' and ext == '.py':
                ...
                for cls_name, cls_type in inspect.getmembers(module):
                    if cls_name == base_class.__name__:
                        continue
                    if inspect.isclass(cls_type) and base_class.__name__ in [item.__name__ for item in inspect.getmro(cls_type)]:
    ...
    def exec_command(self, user_input):
        for cmd in self.commands.values():
            ...
            if match is not None:
                return cmd.get_field_names(), cmd.exec_query(self.handlers)
    ...
    def _get_handler(self, data):
        ...
        if protocol_def.key in self.handlers:
